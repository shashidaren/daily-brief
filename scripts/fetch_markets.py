...
def get_quote(sym):
    ...
    if not hist.empty:
        ...
        if len(hist["Close"]) >= 2: prev = float(hist["Close"].iloc[-2])
    if price is None:
...
def main():
    ...
    for sym,name in TICKERS:
        try:
            ...
            items.append({"ticker":sym,"name":name,"price":price,"change":change,"percent":percent,"exchange":exch})
        except Exception as e:
            items.append({"ticker":sym,"name":name,"price":0,"change":0,"percent":0,"exchange":"N/A","error":str(e)})
    payload={"generatedAt": datetime.now(timezone.utc).isoformat(), "items": items}
    import os; os.makedirs("markets", exist_ok=True)
    with open("markets/markets.json","w",encoding="utf-8") as f: json.dump(payload, f, ensure_ascii=False, indent=2)
